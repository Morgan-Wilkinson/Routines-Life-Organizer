...
struct RoutineItem: Hashable, Identifiable {
    ...
    var progress: Double = 25.0
    // The date of when the entry was last edited.
    var dateLastEdited: Date = Date()
    
    init(name: String, motherGroup: String) {
        self.name = name
        self.motherGroup = motherGroup
    }
}

extension RoutineItem {
    // Single examples
    static var dog = RoutineItem(name: "Walk the dog!", motherGroup: "😣 Chores")
